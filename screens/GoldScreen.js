...
const GoldScreen = () => {
  ...
  useEffect(() => {
    const fetchSchemes = async (house, category) => {
      ...
      try {
        ...
        if (result.Success && Array.isArray(result.SchemeName) && result.SchemeName.length > 0) {
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Invest Now - Gold</Text>
          ...
          {showResult && selectedSchemeData && (
            <View style={styles.card}>
              <Text style={styles.infoText}>
                Fund House: {fundHouseOptions.find(f => f.value === fundHouse)?.label || fundHouse}
              </Text>
              ...
              <View style={styles.buttonRow}>
                <TouchableOpacity
                  style={[
                    styles.actionButton,
                    (!amount || isNaN(amount) || Number(amount) <= 0) && { backgroundColor: '#ccc' }
                  ]}
                  onPress={() => handleBuy('BUY')}
